...
const Home = () => {
  ...
  const getTrendingMoviesData = () => {
    const apiUrl = `${import.meta.env.VITE_SERVER_URL}/api/discover-movies?page=${randomPage}`;
    fetchData(apiUrl, setCarouselMoviesArray);
  };

  const getTrendingDocumentaryData = () => {
    const apiUrl = `${import.meta.env.VITE_SERVER_URL}/api/discover-documetries`;
    fetchData(apiUrl, setCarouselDocumentaryArray);
  };

  const getTrendingAnimeData = () => {
    const apiUrl = `${import.meta.env.VITE_SERVER_URL}/api/discover-animes?page=${randomPage}`;
    fetchData(apiUrl, setCarouselAnimeArray);
  };

  const getTrendingTvData = () => {
    const apiUrl = `${import.meta.env.VITE_SERVER_URL}/api/discover-tv?page=${randomPage}`;
